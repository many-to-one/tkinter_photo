...
void apply_all_adjustments(
    ...
    // float hue_adj, float sat_adj, float lum_adj
    float hsl_h[6], float hsl_s[6], float hsl_l[6] // simplified HSL per-color bands (6 groups)
) {
    ...
    for (int i = 0; i < size; i += 3) {
        ...
        float factor = (luminance < 0.5f) ? shadow_factor : highlight_factor;
        r *= factor;
        g *= factor;
        b *= factor;
        ...
        // Optional: shift hue (wrap around 0–1)
        h += hsl_h[band];
        if (h > 1.0f) h -= 1.0f;
        if (h < 0.0f) h += 1.0f;
